...
Una variable empieza por \texttt{\$} y no se declara con un tipo: PHP infiere el tipo del valor que le asignas (tipado dinámico). Aun así, PHP 8 permite y recomienda declarar tipos en funciones y propiedades para evitar errores. Los tipos básicos son enteros, flotantes, cadenas, booleanos, arrays, objetos y \texttt{null}.
...
var_dump($edad);         // muestra tipo y valor: int(21)
echo gettype($nota);     // 'double'
...
\end{cajacodigo}
\vspace{2pt}
Usa \texttt{var\_dump()} para inspeccionar una variable cuando algo no funcione: te dice su tipo y su contenido, lo que ahorra muchísimo tiempo de depuración. Las constantes, por convención, se escriben en mayúsculas.
