...
Después definimos dos propiedades en \texttt{application.properties}: el secreto con el que se firman los tokens y su tiempo de vida. Para el algoritmo HS256 la clave debe medir al menos 32 bytes (256 bits); usa una cadena larga y aleatoria, y en producción léela de una variable de entorno, nunca del repositorio.

\begin{cajacodigo}[src/main/resources/application.properties --- claves de JWT]
\begin{lstlisting}[style=markup]
# Secreto de firma (>= 32 caracteres para HS256). En produccion: variable de entorno.
app.jwt.secret=cambia-esta-clave-por-una-larga-y-aleatoria-de-32-bytes-minimo
...
Con \texttt{spring.\allowbreak{}jpa.\allowbreak{}hibernate.\allowbreak{}ddl-auto=update}, Hibernate genera estas dos tablas la primera vez que arranca la aplicación.

\begin{cajacodigo}[DDL que genera Hibernate]
...
\item Entidad \texttt{Usuario}, enumerado \texttt{Rol} y \texttt{UsuarioRepository} creados; tablas generadas por Hibernate.
