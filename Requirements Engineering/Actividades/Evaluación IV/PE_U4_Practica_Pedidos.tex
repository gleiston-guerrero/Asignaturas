...
%  Caso: Sistema de Gestion de Pedidos. Modalidad: individual, en clase, con repositorio.
...
\usepackage[colorlinks=true,linkcolor=darkblue,citecolor=darkblue,urlcolor=midblue,
	pdftitle={Prueba Practica Unidad IV - Ingenieria de Requisitos ISR-401},
...
\fancyhead[L]{\footnotesize\sffamily\color{darkblue}Prueba Pr\'actica\, \textbullet\, Unidad IV}
...
\begin{center}
	...
	{\large\bfseries\color{darkblue} PRUEBA PR\'ACTICA --- UNIDAD IV}\\[2pt]
...
\begin{tabularx}{\linewidth}{@{}L{2.6cm} X L{2.6cm} X@{}}
	...
	\hg{Caso} & Sistema de Gesti\'on de Pedidos & \hg{Ponderaci\'on} & 100 puntos (30 + 70) \\
\end{tabularx}
\end{center}
...
% #####################################################################
\section{Caso pr\'actico: Sistema de Gesti\'on de Pedidos}

Una tienda en l\'inea requiere un m\'odulo para gestionar los pedidos de sus clientes. Un \textbf{Cliente} (identificado por c\'edula/RUC, nombre y correo) puede realizar cero o m\'as \textbf{Pedidos}. Cada \textbf{Pedido} (identificador, fecha y total) contiene una o m\'as \textbf{L\'ineas de pedido}, y cada l\'inea referencia un \textbf{Producto} (c\'odigo, descripci\'on, precio, \textit{stock}). Al registrar un pedido, el sistema verifica la disponibilidad de \textit{stock}: si hay existencias, \emph{confirma} el pedido; si no, \emph{notifica el faltante}. Un pedido confirmado puede \emph{despacharse}, luego \emph{entregarse}, o bien \emph{anularse} mientras no haya sido entregado. El sistema debe registrar los pedidos con rapidez y proteger los datos personales del cliente.
...
Construya el \textbf{diagrama de clases UML} del dominio con al menos las clases \textsf{Cliente}, \textsf{Pedido}, \textsf{L\'ineaPedido} y \textsf{Producto}. Cada clase debe incluir sus \textbf{atributos} con tipo, al menos una \textbf{operaci\'on} pertinente y las \textbf{multiplicidades} en los extremos de cada asociaci\'on. Subraye o marque los identificadores.
\end{tarea}

\begin{tarea}{P2. Modelo funcional --- Diagrama de actividades UML \hfill (8 pts)}
Modele el proceso \textbf{``Registrar pedido''} con un \textbf{diagrama de actividades UML} que incluya: nodo inicial, al menos una \textbf{decisi\'on} (\textsf{\textquestiondown stock?}), los dos caminos alternativos (\emph{confirmar} / \emph{notificar faltante}), su convergencia y el nodo final. Use carriles de responsabilidad si distingue actores.
\end{tarea}

\begin{tarea}{P3. Modelo de comportamiento --- M\'aquina de estados UML \hfill (8 pts)}
Elabore la \textbf{m\'aquina de estados UML} del ciclo de vida de un \textsf{Pedido} (p.\,ej. \textsf{Registrado}, \textsf{Confirmado}, \textsf{Enviado}, \textsf{Entregado}, \textsf{Cancelado}). Etiquete cada \textbf{transici\'on} con su \textbf{evento}, incluya al menos una \textbf{condici\'on de guarda} y marque el estado final de aceptaci\'on. Se valora el uso de un superestado (statechart) para factorizar la transici\'on \emph{anular}.
\end{tarea}
...
Redacte \textbf{4 requisitos}: \textbf{2 funcionales} y \textbf{2 no funcionales}. Cada requisito no funcional debe \textbf{nombrar una caracter\'istica de \textit{ISO/IEC} 25010:2023} (p.\,ej. Eficiencia de desempe\~no, Seguridad), fijar una \textbf{m\'etrica} y un \textbf{umbral} (p.\,ej. ``registrar el pedido en $<$ 3 s''). Para cada requisito complete un \textbf{esquema de atributos}: identificador, prioridad, estado, fuente, estabilidad, versi\'on y m\'etodo de verificaci\'on.
\end{tarea}
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\linewidth}{@{}L{3.0cm} C{0.9cm} X X X@{}}
		...
		\rowcolor{rowgray}
		P2. Diagrama de actividades UML & 8 & Decisi\'on de \textit{stock}, ambos caminos, convergencia y nodo final bien modelados. & Flujo correcto pero sin decisi\'on o sin convergencia expl\'icita. & Secuencia lineal sin l\'ogica de control del caso.\\
